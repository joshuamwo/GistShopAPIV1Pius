...
exports.getAllActivitiesAfter = async function (req, res) {
    ...
    try {
        const activity = await activitiesModel.find({ $gt: { _id: req.body.id } }).sort({ time: -1 }).limit(20)
        res.json(activity)
...
exports.getUserActivities = async function (req, res) {
    ...
    try {
        const activity = await activitiesModel.find({ to: req.params.uid }).sort({ time: -1 }).limit(20)
        res.json(activity)
